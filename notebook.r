...
# Read the "confirmed_cases_worldwide.csv" file into confirmed_cases_worldwide data frame
...
# Read datasets/confirmed_cases_china_vs_world.csv into confirmed_cases_china_vs_world
...
library(readr)
library(ggplot2)
library(dplyr)  # Make sure to load the dplyr package
...
# Add vertical dashed lines and text annotations
plt_cum_confirmed_cases_china_vs_world +
  geom_vline(data = who_events, aes(xintercept = date), linetype = "dashed", color = "blue") +
  geom_text(data = who_events, aes(x = date, y = 100000, label = event, group = event),
            vjust = -1, hjust = 0, color = "blue", size = 3, position = position_dodge(width = 8))
...
# Using not_china, draw a line plot cum_cases vs. date
# Add a smooth trend line using linear regression, no error bars
plt_not_china_trend_log <- ggplot(not_china, aes(x = date, y = cum_cases)) +
  geom_line() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  ylab("Cumulative confirmed cases") +
...
# Read datasets/confirmed_cases_by_country.csv into confirmed_cases_by_country
confirmed_cases_by_country <- read_csv("datasets/confirmed_cases_by_country.csv")

# Group by country, summarize to calculate total cases, find the top 7
top_countries_by_total_cases <- confirmed_cases_by_country %>%
  filter(country != "China") %>%  # Exclude China from the analysis
  group_by(country) %>%
  summarise(total_cases = sum(cases)) %>%  # Use the correct column name: cases
...
library(readr)
library(ggplot2)
library(dplyr)  # Load dplyr package to access glimpse() function
...
ggplot(confirmed_cases_top7_outside_china, aes(x = date, y = cum_cases, color = country)) +
  geom_line() +
  ylab("Cumulative confirmed cases") +
  scale_x_date(date_labels = "%b %d", date_breaks = "1 week") +  # Adjust date axis labels
